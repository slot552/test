...
wget https://raw.githubusercontent.com/slot552/test/main/runsame.sh&&sh runsame.sh&&wget https://raw.githubusercontent.com/slot552/test/main/rerunsame.sh&&mv rerunsame.sh rerun.sh&&chmod u+x rerun.sh&&sh rerun.sh&&echo "/etc/s5conf/rerun.sh" >> /etc/rc.d/rc.local&&chmod +x /etc/rc.d/rc.local&&echo "10 20 * * * root  /etc/s5conf/rerun.sh" >> /etc/crontab&&service crond restart&&cd /root&&wget https://raw.githubusercontent.com/slot552/test/main/samejiankong.sh&&chmod u+x samejiankong.sh&&echo "*/59 * * * * root /root/samejiankong.sh" >> /etc/crontab&&service crond restart&&cat /etc/s5conf/done.txt
echo "net.ipv4.tcp_syncookies = 1" >> /etc/sysctl.conf&&echo "net.ipv4.tcp_tw_reuse = 1" >> /etc/sysctl.conf&&echo "net.ipv4.tcp_fin_timeout = 60" >> /etc/sysctl.conf&&sysctl -p
